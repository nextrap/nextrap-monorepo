...
## Libraries, interne Packages und Test-Abhängigkeiten

Jedes veröffentlichte Nextrap-Package deklariert alle direkt verwendeten externen Libraries (einschließlich Lit, Lit-Unterpaketen, TrunkJS und Libraries für veröffentlichte Generatoren) ausschließlich als `peerDependencies` mit einem kompatiblen Semver-Bereich. Bestehende erforderliche Mindestversionen dürfen nicht abgesenkt werden; neue Mindestversionen müssen durch verwendete APIs und Prüfungen begründet sein. Externe Libraries dürfen nicht zusätzlich in `dependencies` stehen und müssen beim Library-Build einschließlich ihrer Subpfade extern bleiben, damit die konsumierende Anwendung eine gemeinsame kompatible Instanz bereitstellt. Dies gilt ebenso für Abhängigkeiten zwischen veröffentlichten `@nextrap/*`-Packages: Basis-, Element-, Layout- und Style-Packages deklarieren direkt benötigte andere Nextrap-Packages ausschließlich als `peerDependencies`, niemals zusätzlich als `dependencies`. Bei einer Umstellung bleiben bestehende kompatible Versionsbereiche erhalten. Reine Demo-, Test- oder Build-Abhängigkeiten bleiben `devDependencies`; lose Zusammenarbeit über Slots oder Events begründet allein keine Pflichtabhängigkeit. Die lokalen Nextrap-Packages werden für Entwicklung und Prüfungen über die npm-Workspaces bereitgestellt. Package-Generatoren müssen direkt benötigte Nextrap-Packages ebenfalls als Peers deklarieren. Ein Peer-Eintrag ersetzt keinen erforderlichen Import zur Custom-Element-Registrierung; Peers und ihre JavaScript-Subpfade bleiben im Library-Build extern. Veröffentlichte Sass-Imports auf andere Packages müssen ebenfalls durch passende Peers gedeckt sein; internes Shadow-DOM-SCSS darf weiterhin beim Build kompiliert werden.
...
- Lifecycle-Callbacks einer Basiskomponente müssen beim Überschreiben immer an die Elternklasse weitergereicht werden. `super.connectedCallback()`, `super.disconnectedCallback()`, `super.firstUpdated(...)`, `super.updated(...)` und vergleichbare Parent-Callbacks sind jeweils genau einmal aufzurufen; das gilt auch bei `disconnected`, `render`/`renderUpdated` und bei vorzeitigen Rückgaben. Bei `render()` muss der Parent-Render ebenfalls aufgerufen und sein Ergebnis gemäß dem Komponentenvertrag erhalten oder in das eigene Template integriert werden. Cleanup darf zusätzlich vor dem `super.disconnectedCallback()` erfolgen, darf den Parent-Aufruf aber nicht überspringen.
- Der Shadow DOM ist so klein wie möglich zu halten und darf nur die funktional notwendigen Basisregeln für die Grundfunktion der Web Component enthalten, etwa internes Layout, Disclosure-Verhalten, Popover-Verhalten, Zustandsdarstellung für die Funktion und erforderliche Overflow-/Positionierungslogik. Alles andere wird außerhalb des Shadow DOM über öffentliche `::part(...)`, Attribute, Klassen, CSS-Variablen und SCSS-Mixins gestylt. Visuelle Regeln wie Farben, Flächen, Rahmen, Schatten, Typografie, Abstände oder responsive Präsentationsvarianten gehören grundsätzlich in die äußere Styling-Schicht. Hintergrund: Styles im Shadow DOM können durch Root- oder Theme-Styles nicht mehr überschrieben werden und verhindern dadurch die gewünschte zentrale Anpassbarkeit. Es sollten immer öffentliche Parts definiert werden, damit äußere Styles die betroffenen Bereiche erreichen können.
- Komponenten, die keinen vorgesehenen Default-Slot besitzen, rendern trotzdem einen Default-Slot mit dem Attribut `NO-DEFAULT-SLOT` als sichtbaren Diagnose-Fallback. In diesen Slot dürfen keine Elemente gelangen. Sichtbarer Inhalt dort bedeutet, dass mindestens ein `slot`-Name im verwendenden Markup falsch ist; die vorgesehenen benannten Slots müssen korrigiert werden.
- `tj-responsive` ist im Theme beziehungsweise in der Website-Struktur bereits gesetzt und wird dort bereitgestellt. Es darf nicht zusätzlich in Komponenten, Component-Templates, Package-Entrypoints oder internen Initialisierungen eingefügt werden. Komponenten verwenden ausschließlich die vorhandene Responsive-API und die von ihr gesetzten Zustände; Registrierung und Einbindung bleiben Aufgabe des Themes beziehungsweise der Website.
- Komponenten (`nte-*` und `ntl-*`) besitzen zwei JavaScript-Entrypoints: `index.ts` exportiert die API von `unstyled.ts` und importiert `default.scss` regulär; der App-Bundler verarbeitet das daraus erzeugte komponentenspezifische Light-DOM-CSS. Der öffentliche Entry Point `/unstyled` darf niemals Light-DOM-SCSS/-CSS direkt oder indirekt importieren, exportieren, re-exportieren, ausgeben oder injizieren, auch nicht durch Side-Effect-Imports, dynamische Imports, eingebundene Runtime-Module oder spätere Lifecycle-/API-Aufrufe. Er enthält ausschließlich Logik, Registrierung und inline eingebundene Shadow-DOM-Styles. Interne Komponentenabhängigkeiten werden im Unstyled-Importgraphen ebenfalls über `/unstyled` geladen; der normale Einstieg ergänzt deren Default-Einstiege. Theme-/Seaming-Entwicklung (insbesondere ThemeJS2) importiert Komponenten ausschließlich über `/unstyled` und materialisiert die öffentliche Sass-Mixin-API explizit im Theme. `index.scss` bleibt eine Sass-API ohne CSS-Ausgabe; `default.scss` materialisiert dieselben Mixins unter komponentenspezifischen `.style-default`-Selektoren. Instanzbezogene funktionale DOM-Geometrie und Zustandswerte bleiben Logik; sie dürfen keine Default-Stylesheets verstecken. Shadow-DOM-Styles bleiben auf den bestehenden Shadow-DOM-Vertrag beschränkt.
...
- Alle Code-Dateien werden nach funktionalen Blöcken kommentiert. Das gilt insbesondere für SCSS, CSS, TypeScript, JavaScript, HTML, PHP sowie Konfigurations- und Markup-Dateien.
- Vor jedem zusammengehörigen Code-Block, jeder Funktion, Methode, Klasse, Komponente, Regelgruppe oder jedem Mixin steht ein kurzer deutscher Kommentar, der den Zweck, das gesteuerte Verhalten und — sofern relevant — den betroffenen Zustand oder die Variante beschreibt. Einzelne Anweisungen oder Deklarationen innerhalb eines zusammengehörigen Blocks benötigen keinen eigenen Kommentar; technische Folge- oder Hilfsanweisungen dürfen mit dem übergeordneten Kommentar gruppiert werden.
- Kommentare dürfen nicht nur den Namen des Elements wiederholen, sondern müssen dokumentieren, warum der Block genau so umgesetzt ist und welche Verantwortung er hat.
- Bei Änderungswünschen von Entwicklern müssen vor der Änderung die vorhandenen Zweckkommentare evaluiert werden. Wenn sich Zweck oder Verhalten ändert, sind die Kommentare anzupassen. Mögliche Seiteneffekte auf andere Komponenten, Module, Varianten, Breakpoints, Slots, Zustände oder öffentliche APIs müssen vor der Umsetzung bewertet werden. Wenn solche Seiteneffekte nicht eindeutig ausgeschlossen werden können, muss zuerst der User gefragt werden; die Änderung darf bis zur Klärung nicht umgesetzt werden.
