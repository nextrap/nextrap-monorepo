...
- Style-/Responsive-Konfiguration: Präsentations-, Layout- und responsive Werte werden nach Möglichkeit über CSS Custom Properties gesteuert und bei Bedarf aus dem effektiven Component Style gelesen. Responsive Zustände kommen aus dem TrunkJS Responsive Framework, nicht aus eigenen Media Queries oder `matchMedia`. Komponenten mit JS-abhängiger Style-Konfiguration reagieren auf `class`-/`style`-Änderungen und lesen ihre effektive Konfiguration neu ein. Details: [Component Style & Responsive Configuration](references/component-style-responsive-configuration.md).
