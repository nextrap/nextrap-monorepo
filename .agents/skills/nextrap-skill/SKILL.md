...
| `@nextrap/style-typography` | Base typographic rules (headings, body, links, …). Uses `style-elements` prose. |
...
**Allowed** (only CSS custom properties):
```css
:root {
  --nt-primary: ...;
  --nt-space-3: ...;
...
This must be safe and not change the Bootstrap page visually:
```scss
@use 'bootstrap';
...
`style-elements` and `style-utils` must compile without importing Sass internals from `style-base`.

### Derived themes compose mixins onto semantic selectors

A derived theme applies Nextrap mixins to native/semantic HTML selectors without requiring Nextrap utility classes in the HTML:
