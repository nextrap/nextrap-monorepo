...
- Horizontal disclosures promote their submenu to a native auto Popover, gaining top-layer placement, light dismiss and Escape handling.
- Vertical disclosures never use Popover; they remain inline in the `details` flow and slide open below the parent.
...
- Horizontal orientation positions the submenu with CSS Anchor Positioning as a native Popover; vertical orientation must keep it inline and slide it below its parent.
- Keep presentation switching driven by the orientation custom properties. An open submenu must move safely between Popover and inline disclosure when the responsive layout changes.
