...
- Submenus never use the native Popover API; they stay inside the native `details` flow so vertical navigation expands inline.
- Horizontal orientation positions submenus with normal CSS (`position: absolute`) and must not move them into the top layer.
...
- Horizontal orientation positions the submenu with CSS custom-property-driven absolute positioning; vertical orientation keeps it static and inline below its parent.
- Keep presentation switching driven by the orientation custom properties. An open submenu must remain a native inline disclosure when the responsive layout changes.
