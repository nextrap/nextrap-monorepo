...
An optional native `caption` is measured and kept fixed above the column header. The component host is the only element that owns the outer border and radius; every nested table element has a zero radius. When horizontal content overflows, `tbody` keeps a dedicated strip below the elevated footer, exposes a styled native scrollbar where supported, and mirrors it with a mobile-visible indicator. `tbody` remains the only actual scroll container. The component never observes or clones the source table. Applications call `refresh()` after structural or layout-metadata changes.
