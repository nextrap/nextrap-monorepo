...
import style from './nte-form-input.scss?inline';
...
@customElement('nte-form-input')
export class NteFormInput extends LitElement {
  ...
  private input: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | null = null;
  ...
  get inputElement(): HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | null {
    return this.input;
  }
  ...
  private updateFloatingState(input: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement) {
    ...
    root.addEventListener('click', (e) => {
      ...
      if (label.tagName === 'LABEL') {
        ...
        if (slot) {
          ...
          const input = elements[0] as HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement;
          if (input) {
            input.focus();
  ...
  override connectedCallback() {
    ...
    if (this.id) {
      ...
      window.addEventListener(`nxa-provide-suggestion-fetcher:\${this.id}`, this._handleProvideFetcherEvent);
    }
  }
  ...
  override disconnectedCallback() {
    ...
    if (this.id) {
      window.removeEventListener(`nxa-provide-suggestion-fetcher:\${this.id}`, this._handleProvideFetcherEvent);
    }
  }
  ...
  private _handleProvideFetcherEvent = (event: Event) => {
    ...
    if (customEvent.detail && typeof customEvent.detail.fetcher === 'function') {
      ...
      if (!this.searchSuggestionsList || this.searchSuggestionsList.length === 0) {
        ...
        console.log(`[NxaFormInput] Received suggestionFetcher via event for ID: \${this.id}`, this.suggestionFetcher);
      } else {
        console.log(
  ...
  override firstUpdated() {
    ...
    if (slot) {
      slot.addEventListener('slotchange', () => {
        const elements = slot.assignedElements();
        this.input = elements[0] as HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement;
        if (this.input) {
          ...
          if (this.input instanceof HTMLInputElement) {
            if (componentUIType === 'search') {
              this.input.addEventListener('keydown', (e: KeyboardEvent) => this._handleSearchInputKeydown(e));
              console.log('[NxaFormInput] Attaching input listener for search');
              this.input.addEventListener('input', () => this._handleSearchInputChange());
              console.log('[NxaFormInput] Attaching focus listener for search');
              this.input.addEventListener('focus', () => this._fetchSuggestions());
              ...
              if (!['checkbox', 'file', 'button', 'submit', 'reset', 'image'].includes(actualInputType)) {
              ...
              if (actualInputType === 'checkbox' || componentUIType === 'checkbox' || componentUIType === 'file') {
          ...
          <div class="pills-and-input-container">
            ${this.selectedPills.map(
              (pill) => html`
                <span class="pill">
                  ${pill}
                  <button class="remove-pill" @click=${() => this._removePill(pill)}>×</button>
                </span>
              `,
            )}
            <slot name="input"></slot>
          </div>
          ${this.showSuggestions && (this.searchSuggestions.length > 0 || this.suggestionLoading)
            ? html` <div class="suggestions-container">
                ${this.suggestionLoading ? html`<div class="suggestion-item loading">Loading...</div>` : ''}
                ${this.searchSuggestions.map(
                  (suggestion) => html`
                    <div class="suggestion-item" @mousedown=${() => this._selectSuggestion(suggestion)}>
                      ${suggestion}
                    </div>
                  `,
                )}
  ...
  private async _fetchSuggestions() {
    if (this.type !== 'search' || !this.input || !(this.input instanceof HTMLInputElement)) return;
    const query = this.input.value.trim();

    if (this.searchSuggestionsList && this.searchSuggestionsList.length > 0) {
      this.searchSuggestions = this.searchSuggestionsList.filter(
        (s) => s.toLowerCase().includes(query.toLowerCase()) && !this.selectedPills.includes(s),
      );
    ...
    try {
      ...
      this.searchSuggestions = suggestions.filter((s) => !this.selectedPills.includes(s));
    } catch (error) {
  ...
  private _removePill(pillValue: string) {
    this.selectedPills = this.selectedPills.filter((pill) => pill !== pillValue);
    this.requestUpdate();
    // Optionally, dispatch an event or update form value here
  }

  private _generatedId = `nxa-form-input-${Math.random().toString(36).substring(2, 9)}`;
