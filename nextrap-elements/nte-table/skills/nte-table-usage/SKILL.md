...
- Use `remote.selectRow|deselectRow|toggleRow` and `remote.selectColumn|deselectColumn|toggleColumn`; accept a zero-based index, element, or stable ID. `clearSelection()` clears Remote selection. Column widths use `getColumnWidth()` and `setColumnWidth()`; `getLayoutState()` returns stable-ID layout state.
- Activate plugins through `features`: `sort`, `resize-columns`, `reorder-columns`, and `reorder-rows`. Sorting supports header `data-sort-type`, cell `data-sort-value`, and `data-sortable="false"`; reorder plugins support `data-reorderable="false"`.
