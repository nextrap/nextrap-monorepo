...
export class NxaNavbar extends LitElement {
  ...
  handleScroll() {
    ...
    if (this.config.stickyOnScroll) {
      if (rect.top <= 0) {
        this.shadowRoot
          .querySelector('#nxa-navbar-header')
          .classList.add('fixed');
    ...
    if (this.config.hideOnScroll) {
      if (
        ...
        scrollTop > this.config.hideThreshold
      ) {
        if (rect.top <= 0) {
          this.shadowRoot
            .querySelector('#nxa-navbar-header')
    ...
    if (this.config.hideTopbarOnScroll) {
      if (
        ...
        if (rect.top <= 0) {
          this.shadowRoot
            .querySelector('#nxa-navbar-topbar')
            .classList.add('hidden');
        }
      } else {
        if (rect.top >= 0) {
  ...
  #renderSlot(slot: NavbarSlot) {
    ...
    const hamburger = html`${hamburgerPosition === slot
      ? this.#renderHamburger()
      : ''}`;

    const scrollingContentOrDefault = html`${this._isScrolling &&
    this.querySelector(`[slot="scroll-${slot}"]`)
      ? html`<slot name="scroll-${slot}"></slot>`
      : html`<slot name="${slot}"></slot>`}`;

    const slotContent = html`${slotInMenu === slot &&
    (this._menuOpen || alwaysShowMobileNav)
      ? html``
  ...
  #renderMenu() {
    ...
    return html`<div class="menu ${menuDirection}">
        ...
        ${slotInMenu === 'left' && (this._menuOpen || alwaysShowMobileNav)
          ? html`<slot name="left"></slot>`
          : ''}
        ${slotInMenu === 'center' && (this._menuOpen || alwaysShowMobileNav)
          ? html`<slot name="center"></slot>`
          : ''}
        ${slotInMenu === 'right' && (this._menuOpen || alwaysShowMobileNav)
    ...
    return html`
      <header id="nxa-navbar-header" class="header ${headerStyle} ">
        ...
        <div id="nxa-navbar-main" class="main">
          ${this.#renderSlot('left')} ${this.#renderSlot('center')}
          ${this.#renderSlot('right')}
        </div>
