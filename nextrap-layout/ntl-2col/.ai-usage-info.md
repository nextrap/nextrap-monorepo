...
@include twoCol.with-main-justify($justify: center);
...
`default-style()` is the default visual baseline and vertically centers `main` through `$mainJustify: center`. By default it includes `with-image-auto-objectfit()` and `with-modifier-classes()`.
...
- `.with-main-justify-start` / `.with-main-justify-center` / `.with-main-justify-end` align the main column content at the top, center or bottom.
