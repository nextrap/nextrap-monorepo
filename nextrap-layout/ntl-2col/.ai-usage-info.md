...
- `demo/demo.scss` shows how semantic classes bind the exported SCSS mixins.
...
- Renders `top`, `main`, `aside` and `bottom` areas in Shadow DOM.
- Uses `BreakPointMixin`; the current mode is reflected as `mode="mobile"` or `mode="desktop"`.
- Uses `SubLayoutApplyMixin` queries to assign direct `.top`, `.bottom`, `.aside` children and direct `p:has(img)` elements.
...
- `.reverse` reverses columns on mobile and desktop.
