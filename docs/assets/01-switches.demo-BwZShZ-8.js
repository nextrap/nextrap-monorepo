...
export default defineDemo({
  ...
  render(root) {
    ...
    const output = root.querySelector<HTMLOutputElement>('[data-theme-output]');
    const themeSwitches = Array.from(root.querySelectorAll<HTMLInputElement>('[data-theme-switch]'));

    if (!preview || !output) return;

    const applyScheme = (dark: boolean) => {
      const scheme = dark ? 'dark' : 'light';
      preview.dataset.ntScheme = scheme;
      preview.style.colorScheme = scheme;
      output.value = dark ? 'Dark Theme' : 'Light Theme';
    ...
    themeSwitches.forEach((input) => {
      input.addEventListener('change', () => applyScheme(input.checked));
    });
