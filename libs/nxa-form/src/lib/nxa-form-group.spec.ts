...
import { NxaFormGroup } from './nxa-form-group';

describe('NxaFormGroup', () => {
  it('should create an element', () => {
    const el = new NxaFormGroup();
    expect(el).toBeInstanceOf(NxaFormGroup);
  });

  it('renders with default properties', async () => {
    const { container } = render(html`
      <nxa-form-group data-testid="form-group"></nxa-form-group>
    `);
  ...
  it('slots content', async () => {
    const { container } = render(html`
      <nxa-form-group data-testid="form-group-slotted">
        <div>Slotted Content</div>
      </nxa-form-group>
    `);
    const element = container.querySelector(
      '[data-testid="form-group-slotted"]'
    );
    expect(element).not.toBeNull();
